...
for itr in range(2,14):
    ...
    url = 'https://stats.nba.com/teams/boxscores-advanced/?Season=' + temp_year + '&SeasonType=Regular%20Season'
    browser.get(url)
    #browser.find_element_by_xpath('/html/body/main/div[2]/div/div[2]/div/div/div[1]/div[3]/div/div/label/select/option[1]').click()
    browser.find_element_by_xpath('/html/body/main/div[2]/div/div[2]/div/div/nba-stat-table/div[1]/div/div/select/option[1]').click()
    ...
    big_dict[year] = {}
    #print(big_stats)
    for i in range(len(big_stats)):
        #print(big_stats[i][7], big_stats[i][8])
        if big_stats[i][0] not in big_dict[year]:
            big_dict[year][big_stats[i][0]] = []
        big_dict[year][big_stats[i][0]].append((big_stats[i][19], big_stats[i][20]))
...
pickle.dump(big_dict, open("pace_pie.p", "wb"))
